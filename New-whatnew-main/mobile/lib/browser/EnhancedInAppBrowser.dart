...
class _InstagramBrowserScreenState extends State<_InstagramBrowserScreen>
                    ...
                    child: Container(
                      ...
                      child: InAppWebView(
                        ...
                        shouldOverrideUrlLoading: (controller, navigationAction) async {
                          ...
                          // Handle intent:// schemes (Android app links)
                          if (url.startsWith('intent://')) {
                            await _handleIntentUrl(url);
                            return NavigationActionPolicy.CANCEL;
                          }
                          
                          // Handle custom app schemes
                          if (url.startsWith('instagram://') || 
                            ...
                            await _handleAppScheme(url);
                            return NavigationActionPolicy.CANCEL;
                          }
                          
                          // Handle market:// and play store links
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        if (_currentUrl.contains('instagram.com')) ...[
                          Icon(
                            Icons.camera_alt,
                            size: 12,
                          ...
                          const SizedBox(width: 4),
                        ] else if (_currentUrl.contains('twitter.com')) ...[
                          Icon(
                            Icons.flutter_dash,
                            size: 12,
  ...
  /// Handles intent:// URLs (Android app links)
  Future<void> _handleIntentUrl(String intentUrl) async {
    try {
      // Extract the fallback URL from intent URL
      final uri = Uri.parse(intentUrl);
      
      // Look for fallback URL in the intent
      String? fallbackUrl;
      
      // Check for S.browser_fallback_url parameter
      ...
      // Check for fall_back_to_web parameter
      if (fallbackUrl == null && intentUrl.contains('fall_back_to_web=true')) {
        // Extract the original URL from the intent
        final originalMatch = RegExp(r'intent://([^#]+)').firstMatch(intentUrl);
        if (originalMatch != null) {
          final path = originalMatch.group(1)!;
          // Check if it contains a domain we can use
          if (path.contains('instagram.com')) {
            fallbackUrl = 'https://$path';
          }
        }
      }
      
      // Try to extract web URL from the intent
      if (fallbackUrl == null) {
        // Look for common web URL patterns in the intent
        final webUrlMatch = RegExp(r'https?://[^\s;]+').firstMatch(intentUrl);
        if (webUrlMatch != null) {
          fallbackUrl = webUrlMatch.group(0);
        }
      }
      
      if (fallbackUrl != null && fallbackUrl.isNotEmpty) {
        // debugPrint('Using fallback URL: $fallbackUrl');
        // Load the fallback URL in the current WebView
        await _webViewController?.loadUrl(urlRequest: URLRequest(url: WebUri(fallbackUrl)));
      } else {
        // Show user-friendly message and offer options
        _showUrlNotSupportedDialog(intentUrl);
      }
    } catch (e) {
  ...
  /// Handles custom app schemes
  Future<void> _handleAppScheme(String appUrl) async {
    try {
      ...
      if (await canLaunchUrl(uri)) {
        ...
        // App not installed, provide fallback options
        _showAppNotInstalledDialog(appUrl);
      }
    } catch (e) {
      // debugPrint('Error handling app scheme: $e');
      _showAppNotInstalledDialog(appUrl);
    }
  }
  ...
  void _showUrlNotSupportedDialog(String url) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Link Not Supported'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('This link requires a specific app to open.'),
            const SizedBox(height: 16),
            Text(
              'Link: ${_extractDomainFromUrl(url)}',
              style: Theme.of(context).textTheme.bodySmall?.copyWith(
                color: Colors.grey[600],
              ),
            ),
  ...
  void _showAppNotInstalledDialog(String appUrl) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('$appName is required to open this link. Would you like to install it?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('Cancel'),
  ...
  /// Enhanced URL extraction for better display
  String _extractDomainFromUrl(String url) {
    try {
      if (url.startsWith('intent://')) {
        // Extract domain from intent URL
