...
// UNUSED - Custom delegate for tab bar with ValueNotifier support
// class _SliverTabBarWithNotifierDelegate extends SliverPersistentHeaderDelegate {
  final ValueNotifier<TabController?> tabControllerNotifier;
  final CategoryProvider categoryProvider;
  TabBar? _cachedTabBar;

  _SliverTabBarWithNotifierDelegate({
    required this.tabControllerNotifier,
    required this.categoryProvider,
  });

  TabBar _buildTabBar(TabController? tabController) {
    if (tabController == null) {
      return const TabBar(
        tabs: [
          Tab(text: 'Loading...'),
  ...
  double get minExtent {
    // Use TabBar's preferred height which is standard for Material Design
    return _cachedTabBar?.preferredSize.height ?? kToolbarHeight;
  }

  @override
  double get maxExtent {
    // Same as minExtent for consistent height
    return _cachedTabBar?.preferredSize.height ?? kToolbarHeight;
  }
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    return Container(
      color: Colors.white,
      child: ValueListenableBuilder<TabController?>(
        valueListenable: tabControllerNotifier,
        builder: (context, tabController, child) {
          _cachedTabBar = _buildTabBar(tabController);
          return _cachedTabBar ?? const SizedBox.shrink();
        },
      ),
    );
  }
  ...
  @override
  bool shouldRebuild(_SliverTabBarWithNotifierDelegate oldDelegate) {
    return tabControllerNotifier != oldDelegate.tabControllerNotifier ||
           categoryProvider != oldDelegate.categoryProvider;
  }
}
